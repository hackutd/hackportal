...
import HackerpackDropdownMenuMobile from './HackerpackDropdownMenuMobile';
...
export default function HackerpackDisplay({ content, notionRootId }: Props) {
  ...
  if (hackerpackSettings.sidebar && hackerpackSettings.mainContent === 'markdown') {
    ...
    const rootObj = content.block[notionRootId];
    ...
    actualSidebarContent = [];
    const firstObj = content.block[blockList[0]];
    let currH1 = {
    ...
    for (let i = 1; i < blockList.length; i++) {
      const currObj = content.block[blockList[i]];
